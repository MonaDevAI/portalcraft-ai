...
$tool = Join-Path $root 'tools\PortalCraft.RepoTool'
...
$output = Join-Path $repository '.portalcraft-ai'
$generateArguments = @(
    'run',
    '--project', $tool,
    '--',
...
if ($LASTEXITCODE -ne 0) {
    throw 'PortalCraft AI repository generation failed.'
}
...
if ($LASTEXITCODE -ne 0) {
    throw 'PortalCraft AI PR scenario generation failed.'
}

$environmentFile = Join-Path $output 'client\.env.portalcraft-ai'
@"
VITE_PRODUCT_NAME=$ProductName
VITE_ASSISTANT_NAME=$AssistantName
"@ | Set-Content -Path $environmentFile -Encoding utf8

Write-Host "PortalCraft AI onboarding generated at $output"
